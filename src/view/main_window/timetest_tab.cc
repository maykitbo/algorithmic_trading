...
void MainWindow::TimeTestInit() {
  ...
  layers_frame->GetBackground()->SetXTextFunc(
      [](double x) { return QString::number((long)x); });
  ...
  connect(ui_->t_hide_button, &QPushButton::clicked, this,
          &MainWindow::TimeTestHideButton);
  connect(ui_->t_clear_button, &QPushButton::clicked, this,
          &MainWindow::TimeTestClearButton);
  connect(ui_->t_file_button, &QPushButton::clicked, this,
          &MainWindow::TimeTestFileButton);
  connect(ui_->start_test_button, &QPushButton::clicked, this,
          &MainWindow::TimeTestStartButton);
  connect(ui_->cs_subj_button, &QPushButton::clicked, this,
          &MainWindow::CSSubjButton);
  connect(ui_->subj_minus_button, &QPushButton::clicked, this,
          &MainWindow::SubjMinusButton);
  connect(ui_->subj_plus_button, &QPushButton::clicked, this,
          &MainWindow::SubjPlusButton);
  connect(
      ui_->t_points,
      static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
      [this](int value) { ui_->t_partitions->setMaximum(value - t_size_); });
}
...
void MainWindow::TimeTestStartButton() {
  ...
  facade_.TimeCompare(
      ...
      [this, k, &vg_step, &vg_i](const InterpolationTime::result_data &result) {
        ...
        if (++vg_i > 2) {
          ui_->t_graph_widget->GetBackground()->SetVerticalGridStep(
              result[0].second - vg_step);
        }
...
void MainWindow::CSSubjButton() {
  if (ui_->cs_subj_button->text() == "+") {
    ...
    ui_->cs_subj_button->setText("+");
    QString text =
        "<font style='text-decoration: line-through;'>Cubic spline</font>";
...
void MainWindow::SubjPlusButton() {
  ...
  QLabel *label =
      new QLabel("Newton " + QString::number(t_subj_spin_boxes_.size()));
