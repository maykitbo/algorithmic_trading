...
void MainWindow::InterpolationInit() {
  ...
  connect(ui_->i_hide_button, &QPushButton::clicked, this,
          &MainWindow::InterpolationHideButton);
  connect(ui_->i_clear_button, &QPushButton::clicked, this,
          &MainWindow::InterpolationClearButton);
  connect(ui_->i_file_button, &QPushButton::clicked, this,
          &MainWindow::InterpolationFileButton);
  connect(ui_->CS_button, &QPushButton::clicked, this,
          &MainWindow::CubicSplineButton);
  connect(ui_->N_button, &QPushButton::clicked, this,
          &MainWindow::NewtonPolynomialButton);
  connect(layers_frame, &Graph::Frame::GraphRemoved, this,
          &MainWindow::InterpolationGraphRemove);
  connect(ui_->calc_button, &QPushButton::clicked, this,
          &MainWindow::InterpolationCalcButton);
}
...
void MainWindow::InterpolationRawData(Points &data) {
  ui_->i_date_time_edit->setMinimumDateTime(
      QDateTime::fromSecsSinceEpoch(Convert::ToSeconds(data[0].first)));
  ui_->i_date_time_edit->setMaximumDateTime(
      QDateTime::fromSecsSinceEpoch(Convert::ToSeconds(data.back().first)));
...
void MainWindow::NewtonPolynomialButton() {
  ...
  auto data =
      facade_.NewtonPolynomialData(ui_->i_points_spin_box->value(), degree);
  ui_->i_graph_widget->AddGraph(data, "Newton " + QString::number(degree),
                                false, true);
  ++i_graphs_;
  if (i_graphs_ == i_max_graphs_) {
    ui_->i_add_graph_frame->setEnabled(false);
...
void MainWindow::InterpolationFileButton() {
  InterpolationFileRead(QFileDialog::getOpenFileName(
...
void MainWindow::InterpolationFileRead(const QString &filename) {
  ...
  ui_->i_file_info_lable->setText(
      FileNameString(filename, result.second.size()));
