...
BackgroundFrame::BackgroundFrame(Parameters &params, QWidget *parent)
    : AbstractFrame(parent), ui_(new Ui::BackgroundFrame), Background(params) {
  ui_->setupUi(this);
  ...
  LineStyleHelper(ui_->grid_button, ui_->grids_spin, ui_->grids_box,
                  &grid_pen_);
  ...
  connect(ui_->font_box, &QFontComboBox::currentFontChanged, this,
  ...
  connect(ui_->x_step_spin, QOverload<qreal>::of(&QDoubleSpinBox::valueChanged),
          this, [this](qreal value) {
            SetVerticalGridStep(value);
            emit ReDraw();
          });
  connect(ui_->y_step_spin, QOverload<qreal>::of(&QDoubleSpinBox::valueChanged),
          this, [this](qreal value) {
...
void BackgroundFrame::Update() {
  ...
  UpdateOneGridSpin(ui_->y_step_spin, GetHorizontalGridStep(),
                    p_.GetYDistance());
  Draw();
}

void BackgroundFrame::UpdateOneGridSpin(QDoubleSpinBox *spin, qreal value,
                                        qreal distance) {
...
BackgroundFrame::~BackgroundFrame() { delete ui_; }
