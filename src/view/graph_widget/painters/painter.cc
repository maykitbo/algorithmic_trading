...
Painter::Painter(Parameters &params,
            const QString &name,
...
Painter::Painter(Parameters &params,
            ...
            const QPen &graph_pen,
            QImage::Format format)
    : AbstractPainter(params, NameFormat(name, data.size()), params.GraphSize(), format)
    , points_pen_(ColorInvert(graph_pen.color()), graph_pen.width() + 1)
    , graph_pen_(graph_pen)
    , data_(data)
{
...
void Painter::DrawingPoints(bool draw_points)
    { draw_points_ = draw_points; }

void Painter::DrawingLines(bool draw_lines)
    { draw_lines_ = draw_lines; }

void Painter::SetPointsPen(QPen pen)
    { points_pen_ = pen; }

void Painter::SetGraphPen(QPen pen)
    { graph_pen_ = pen; }
...
const QPen &Painter::GetGraphPen() const noexcept
    { return graph_pen_; }

const QPen &Painter::GetPointsPen() const noexcept
    { return points_pen_; }

qreal Painter::MinX() const noexcept
    { return min_x_; }

qreal Painter::MaxX() const noexcept
    { return max_x_; }

qreal Painter::MinY() const noexcept
    { return min_y_; }

qreal Painter::MaxY() const noexcept
    { return max_y_; }
