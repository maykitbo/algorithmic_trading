...
WidgetFrame::WidgetFrame(bool attach_frame, QWidget *parent)
  ...
  layout_->addItem(
      ...
      0, 2, 1);
  layout_->addItem(
      new QSpacerItem(0, 0, QSizePolicy::Preferred, QSizePolicy::Expanding), 1,
      1, 1, 1);

  if (attach_frame) AttachFrame();
}
...
void WidgetFrame::AddGraph(const data_t &data, const QString &name, bool points,
                           bool removeable) {
  frame_->AddGraph(name, data, points, removeable);
}

void WidgetFrame::AddGraph(data_t &&data, const QString &name, bool points,
                           bool removeable) {
  frame_->AddGraph(name, data, points, removeable);
}
...
void WidgetFrame::Draw() { frame_->Draw(); }
...
Frame *WidgetFrame::GetFrame() { return frame_; }
