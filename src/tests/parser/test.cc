...
void WeightTest(const std::string &file,
                const std::function<void(fp_type)> &func) {
...
#define __PARSER_TEST(name, size) \
  TEST(Parser, name) { OneFileTest("materials/" #name ".csv", size); }

#define __BAD_PARSER_TEST(name) \
  TEST(Parser, bad_##name) { BadFileTest("materials/bad_csv/" #name ".csv"); }

#define __WEIGHT_TEST(name, func) \
  TEST(Parser, weight_##name) { WeightTest("materials/" #name ".csv", func); }
