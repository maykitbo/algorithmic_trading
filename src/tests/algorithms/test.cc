...
void FileTest::CubicSplineCalc()
{
    ...
    Real step = (points_.back().first - points_.front().first) / static_cast<Real>(max);
    Real time = points_.front().first;
    for (int k = 0; k < max; ++k, time += step)
    {
        ExpectedNumber(CubicSpline::Calc(points_, time));
    }
    EXPECT_TRUE(std::isnan(static_cast<fp_type>(CubicSpline::Calc(points_, points_.back().first + 1.0))));
    EXPECT_TRUE(std::isnan(static_cast<fp_type>(CubicSpline::Calc(points_, points_.front().first - 1.0))));
}
...
void FileTest::NewtonPolynomialSolve()
{
    for (int g = 0; g < points_.size() / 2; ++g)
    {
        ...
        for (int k = 0; k < 3; ++k)
        {
            unsigned degree = Random::Int<unsigned>(2, std::min<unsigned>(points_.size() - 2, 16));
...
void FileTest::NewtonPolynomialCalc()
{
    ...
    EXPECT_TRUE(std::isnan(static_cast<fp_type>(NewtonPolynomial::Calc(points_, 2, points_.back().first + 1.0))));
    EXPECT_TRUE(std::isnan(static_cast<fp_type>(NewtonPolynomial::Calc(points_, 2, points_.front().first - 1.0))));
}
