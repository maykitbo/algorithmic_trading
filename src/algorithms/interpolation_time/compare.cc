...
void InterpolationTime::Compare(const PointsDec &points, bool cubic_spline,
  ...
  for (int k = size; k <= points_count; k += step) {
    ...
    if (cubic_spline) {
      result[i].second = k;
      result[i++].first =
          Time::Test<Time::mcs>([&] { CubicSpline::Solve(points, k); }, N);
    }
    for (auto degree : newtons_degree) {
      result[i].second = k;
      result[i++].first = Time::Test<Time::mcs>(
          [&] { NewtonPolynomial::Solve(points, degree, k); }, N);
    }
    func(result);
  }
}
...
unsigned InterpolationTime::OptimalIterations(unsigned size, unsigned k) {
  ...
  static interval_map<unsigned, unsigned> iterations = []() {
    ...
    itarations.insert(
        make_pair(interval<unsigned>::right_open(1000, 10000), 5));
    itarations.insert(
        make_pair(interval<unsigned>::right_open(10000, 100000), 3));
    itarations.insert(
        make_pair(interval<unsigned>::right_open(
                      100000, std::numeric_limits<unsigned>::max()),
                  1));
    return itarations;
  }();
