...
NewtonPolynomial::NewtonPolynomial(const PointsDec &points)
    : points_(points), size_(points_.size()), degree_(0) {
  if (size_ < 2) {
...
bool NewtonPolynomial::CreatePolynomial(unsigned degree) {
  ...
  std::vector<std::vector<Real>> divided_differences_(
      degree_, std::vector<Real>(degree_));
  for (int k = 0; k < size_; ++k) {
    ...
    for (unsigned j = 1; j < m; ++j) {
      for (unsigned i = 0; i < m - j; ++i) {
        divided_differences_[i][j] =
            (divided_differences_[i + 1][j - 1] -
             divided_differences_[i][j - 1]) /
            (points_[i + j + k].first - points_[i + k].first);
      }
...
Real NewtonPolynomial::Interpolate(Real x_interpolated, unsigned k) {
  ...
  for (unsigned i = 1; i <= n; ++i) {
    result =
        (result * (x_interpolated - points_[n - i + k].first)) + coef[n - i];
  }
  return result;
}
...
PointsDec NewtonPolynomial::Solve(const PointsDec &points, unsigned degree,
                                  unsigned N) {
  NewtonPolynomial np(points);
  return np.Solve(N, degree);
}

Real NewtonPolynomial::Calc(const PointsDec &points, unsigned degree,
                            Real time) {
  NewtonPolynomial np(points);
