...
CubicSpline::CubicSpline(const PointsDec &points)
  ...
  lambda[0] = 1.0;
  for (int i = 1; i < N; ++i) {
    lambda[i] = 2.0 * (points_[i + 1].first - points_[i - 1].first) -
                h[i - 1] * mu[i - 1];
    mu[i] = h[i] / lambda[i];
...
PointsDec CubicSpline::Solve(unsigned N) {
  ...
  for (unsigned i = 0; i < N; ++i, x += step) {
    ...
    Real y_i =
        points_[j].second + b_[j] * dx + c_[j] * dx * dx + d_[j] * dx * dx * dx;
...
Real CubicSpline::Calc(Real time) {
  ...
  Real x_i = (time - points_.front().first) * (Real)(size_ - 1) /
             (points_.back().first - points_.front().first);
  int j = (int)x_i;
  Real dx = x_i - j;
  Real y_i =
      points_[j].second + b_[j] * dx + c_[j] * dx * dx + d_[j] * dx * dx * dx;
